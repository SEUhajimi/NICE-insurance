<template>
  <div class="page">
    <div class="page-header">
      <h1 class="page-title">Home Policies</h1>
    ...
    <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="Actions" width="160" fixed="right">
        <template #default="{ row }">
          ...
          <el-button size="small" type="danger" @click="handleDelete(row.hpId)">Delete</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="dialogVisible" :title="isEdit ? 'Edit Home Policy' : 'Add Home Policy'" width="500">
...
const defaultForm = { hpId: null, sdate: '', edate: '', amount: 0, status: 'C', hjbCustomerCustId: null }
const form = ref({ ...defaultForm })

const loadData = async () => {
  loading.value = true
  const res = await homePolicyApi.findAll()
...
const handleSubmit = async () => {
  if (isEdit.value) { await homePolicyApi.update(form.value); ElMessage.success('Updated') }
  else { await homePolicyApi.add(form.value); ElMessage.success('Added') }
  dialogVisible.value = false; loadData()
}

const handleDelete = async (id) => {
  await ElMessageBox.confirm('Delete this policy?', 'Warning', { type: 'warning' })
  await homePolicyApi.delete(id); ElMessage.success('Deleted'); loadData()
}
